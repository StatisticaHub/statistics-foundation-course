...
<QuickQuiz
  client:load
  question="Which quantity describes a population?"
  options={["Statistic", "Parameter", "Sample mean only", "Observed frequency only"]}
  correct={1}
  explanation="A parameter describes a population, while a statistic is calculated from a sample."
/>
